...
- PWA: 홈 화면 설치, 오프라인 동작, 파일 관리자 "공유"로 txt 받기(Web Share Target)
  ...
  - 카카오톡·네이버·인스타그램 같은 인앱 브라우저(WebView)는 설치 신호가 오지 않으므로, 대신 "이 브라우저에서는 앱으로 설치할 수 없습니다" 카드와 "브라우저에서 열기" 버튼을 보입니다. Android는 어느 인앱 브라우저든 `intent://` 스킴으로 Chrome을 먼저 엽니다(삼성 기기도 Chrome, 아래 삼성 인터넷 항목 참고). 카카오톡의 `kakaotalk://web/openExternal` 스킴(비공식)과 라인의 `openExternalBrowser=1` 파라미터(공식)는 기기의 기본 브라우저로 열어 주므로 Chrome이 없거나 막혔을 때의 다음 방법으로 씁니다. 1.5초 안에 화면이 바뀌지 않거나 intent의 fallback 주소로 페이지가 다시 뜨면(진행 단계를 `sessionStorage`에 남겨 알아냅니다) 자동으로 다음 방법으로 넘어가고, iOS 일반 WebView처럼 방법이 없거나 다 실패하면 주소를 클립보드에 복사해 두고 붙여넣기나 "다른 브라우저로 열기" 메뉴를 안내합니다. 자동으로 보내지는 않습니다
  ...
  - 그래도 삼성 인터넷의 WebAPK는 targetSdk가 낮아 Android 14 이상에서 Play 프로텍트 "안전하지 않은 앱 차단됨" 경고가 뜨고(더 알아보기 → 무시하고 설치로 통과는 가능), 설치한 뒤에도 상태 바와 제스처 핸들 영역이 검은 띠로 남습니다(삼성 껍데기가 시스템 바를 화면 밖에 두고 밝은 `theme_color`를 다크 모드에서 거부). 둘 다 앱에서 고칠 수 없으므로 삼성 인터넷에서는 설치 카드가 "Chrome에서 설치"를 권하고 "여기서 설치"를 보조로 남깁니다. 삼성 인터넷과 Chrome은 저장소가 분리되어 있어 삼성 인터넷 탭에서 넣은 책은 Chrome 설치본으로 옮겨지지 않습니다. 참고: [SamsungInternet/support#123](https://github.com/SamsungInternet/support/issues/123)
