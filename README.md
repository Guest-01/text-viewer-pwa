...
- PWA: 홈 화면 설치, 오프라인 동작, 파일 관리자 "공유"로 txt 받기(Web Share Target)
  ...
  - 카카오톡·네이버·인스타그램 같은 인앱 브라우저(WebView)는 설치 신호가 오지 않으므로, 대신 "이 브라우저에서는 앱으로 설치할 수 없습니다" 카드와 "브라우저에서 열기" 버튼을 보입니다. 카카오톡은 `kakaotalk://web/openExternal` 스킴(비공식), 라인은 `openExternalBrowser=1` 파라미터(공식), 그 외 Android는 `intent://` 스킴으로 Chrome을 열고(삼성 기기도 Chrome, 아래 삼성 인터넷 항목 참고), iOS 일반 WebView처럼 방법이 없거나 1.5초 안에 화면이 바뀌지 않으면 주소를 클립보드에 복사해 두고 붙여넣기나 "다른 브라우저로 열기" 메뉴를 안내합니다. 자동으로 보내지는 않습니다
